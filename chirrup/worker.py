...
class Worker:
    ...
    def _handle_forward_one_decode_phase(self, task_data: TaskData, slot_pos: int):
        ...
        if new_token in task.stop_tokens:
            ...
            task.output_queue.put_nowait((task.task_id, "token_generated", (new_token, new_text)))
        ...
        if len(task.generated_tokens) >= task.max_tokens:
            ...
            self.alpha_presence_vector[slot_pos, new_token] = torch.tensor(
    ...
    def _run_forward_one(self, decode_offset: Tuple[int, int]):
        ...
        # 模型前向传播
        out = self.model.forward_batch(next_tokens, forward_state)
